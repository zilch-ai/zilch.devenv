...
function scoop_exists()
{
    if ! powershell.exe -Command "Get-Command scoop" &>/dev/null; then
        error "Scoop is not installed or not available in PATH. Please ensure Scoop is properly installed."
        return 1
    fi
}
...
function scoop_self()
{
    ...
    scoop update
    if [ $? -ne 0 ]; then
        error "Failed to update scoop. Please check your configuration or internet connection."
        return 1
    fi
}
...
function scoop_apps()
{
    ...
    || { error "Failed to list installed scoop apps."; return 1; }
